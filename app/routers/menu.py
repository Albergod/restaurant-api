...
async def get_featured_products(table_qr: Optional[str] = None, db: AsyncSession = Depends(get_db)):
    restaurant_id = None
    if table_qr:
        result = await db.execute(select(Table.restaurant_id).where(Table.qr_code == table_qr))
        restaurant_id = result.scalar_one_or_none()
...
async def get_promotions(table_qr: Optional[str] = None, db: AsyncSession = Depends(get_db)):
    restaurant_id = None
    if table_qr:
        result = await db.execute(select(Table.restaurant_id).where(Table.qr_code == table_qr))
        restaurant_id = result.scalar_one_or_none()
